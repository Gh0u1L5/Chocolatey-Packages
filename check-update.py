...
def check_latest_version() -> Tuple[str, str, str]:
    r = requests.get('https://www.bitcomet.com/en/archive')
    if r.status_code != 200:
        raise Exception(f'Failed to download BitComet archive list')

    table = etree.HTML(r.text).find('.//table[@class="bc_releases"]')
    for row in table.find('tbody').iter('tr'):
        url = row.find('td[@class="bc_table_version"]/a').get('href')
        file = row.find('td[@class="bc_table_version"]/a').text
        checksums = row.find('td[@class="bc_table_md5"]/a').get('data-content')

        # Parse version
        match = re.search(r'BitComet_([\d\.]+)_setup\.exe', file)
        if not match:
            continue
        version = match.group(1)

        # Parse checksum
        match = re.search(r'<p>([a-f0-9]{64})<\/p>', checksums)
        if not match:
            continue
